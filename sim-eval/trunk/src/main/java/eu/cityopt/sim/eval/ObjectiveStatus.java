...
public class ObjectiveStatus implements PartiallyComparable<ObjectiveStatus> {
	...
	/**
	 * Values of objective functions, in the same order as the constructor arguments.
	 */
	public final double[] objectiveValues;

	/**
	 * Values of objective functions, with signs flipped so that smaller values are preferred.
	 */
	public final double[] asMinGoalValues;

	public ObjectiveStatus(MetricValues context, ObjectiveExpression singleObjective)
	...
	public ObjectiveStatus(MetricValues context, Collection<ObjectiveExpression> objectives)
			throws ScriptException, InvalidValueException {
		this.namespace = context.getResults().getNamespace();
	...
	public ObjectiveStatus(ConstraintStatus constraintStatus, ObjectiveStatus objectiveStatus) {
	...
	public Integer compareTo(ObjectiveStatus other) {
		if (namespace != other.namespace) {
			throw new IllegalArgumentException("Cannot compare between different namespaces");
		}
		return PartialComparisons.compare(asMinGoalValues, other.asMinGoalValues);
	}
